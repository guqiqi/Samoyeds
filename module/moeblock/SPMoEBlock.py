...
from module.linear.SPDenseLinear import SPDenseLinear
...
class SPMLP(nn.Module):
    def __init__(
        ...
        self.w1 = SPDenseLinear(original, in_features=self.ffn_dim, out_features=self.hidden_dim)
        self.w2 = SPDenseLinear(original, in_features=self.hidden_dim, out_features=self.ffn_dim)
        self.w3 = SPDenseLinear(original, in_features=self.ffn_dim, out_features=self.hidden_dim)

        self.act_fn = act_fn

    def forward(self, hidden_states, routing_weights):
        batch_size, hidden_dim = hidden_states.shape
        current_hidden_states = self.act_fn(self.w1(hidden_states).T) * self.w3(hidden_states).T
        current_hidden_states = self.w2(current_hidden_states)
        return routing_weights * current_hidden_states.T[0:batch_size, :]


class SPMoEBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        self.experts = nn.ModuleList([SPMLP(self.ffn_dim, self.hidden_dim, original=original) for _ in range(self.num_experts)])
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for expert_idx in range(self.num_experts):
            ...
            input_list.append(current_state)
            weight_list.append(routing_weights[top_x_list, idx_list, None])
            
        for expert_idx in range(self.num_experts):
            current_hidden_states = expert_layer(input_list[expert_idx], weight_list[expert_idx])
            ...
            final_hidden_states.index_add_(0, top_x_list[expert_idx], current_hidden_states.to(hidden_states.dtype))
