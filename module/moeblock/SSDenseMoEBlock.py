...
from module.linear.SSLinear import SSLinear
...
class SSMLP(nn.Module):
    def __init__(
        ...
        self.w1 = SSLinear(original, in_features=self.ffn_dim, out_features=self.hidden_dim)
        self.w2 = SPDenseLinear(original, in_features=self.hidden_dim, out_features=self.ffn_dim)
        self.w3 = SSLinear(original, in_features=self.ffn_dim, out_features=self.hidden_dim)
    ...
    def forward(self, hidden_states, input_idx, routing_weights):
        ...
        current_hidden_states = self.w2(current_hidden_states.T)
        return routing_weights * current_hidden_states.T[0:batch_size, :]


class SSDenseMoEBlock(nn.Module):
    """
