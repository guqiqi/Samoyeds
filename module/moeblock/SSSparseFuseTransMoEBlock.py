...
from module.linear.SPSparseWeightedTransLinear import SPDenseWeightedLinear
from module.linear.SSTransLinear import SSTransLinear

from module.moeblock.MoEBlock import MoEBlock, MLP

class SSFusedTransMLP(nn.Module):
    def __init__(
        self, 
        ffn_dim: int,
        hidden_dim: int,
        ...
        self.w1 = SSFusedSiluTransLinear(original, in_features=self.ffn_dim, out_features=self.hidden_dim)
        self.w2 = SPDenseWeightedLinear(original, in_features=self.hidden_dim, out_features=self.ffn_dim)
    ...
    def forward(self, hidden_states, input_idx, routing_weights):
        ...
        current_hidden_states = self.w2(current_hidden_states, input_idx, hidden_states.shape[0], routing_weights.T)
        return current_hidden_states


class SSSparseFuseTransMoEBlock(nn.Module):
    """
    ...
    def __init__(
        ...
        self.experts = nn.ModuleList([SSFusedTransMLP(self.ffn_dim, self.hidden_dim, original=original) for _ in range(self.num_experts)])
