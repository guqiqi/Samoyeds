...
from module.util import sparsifier, N, M, vector_length, padding_weight, padding_idx
import samoyeds_kernel

def sparse_dense_mul_transpose_sparse_weighted_dispatch(A_sparse_values, A_sparse_indices, A_sparse_metadata,
                                                B_values, B_indices, routing_weights, m, k, n, batch_size):
    output = samoyeds_kernel.spmm_weighted_sparse_trans(   A_sparse_values,  # lhs_matrix, compressed values
...
class SPDenseWeightedLinear(nn.Module):
    ...
    def forward(self, input, idx, batch_size, routing_weights):
        idx = padding_idx(idx, 64)
        routing_weights = padding_weight(routing_weights, 64)
        return sparse_dense_mul_transpose_sparse_weighted_dispatch(self.weight, self.indices, self.metadata, input, idx, routing_weights,
                                          self.weight.shape[0] * 2,
                                          self.weight.shape[1] * 2, input.shape[0], batch_size)
