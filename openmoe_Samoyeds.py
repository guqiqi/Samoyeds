...
from openmoe.modeling_openmoe_samoyeds import SSSparseMLP, sparsemoeblock_to_ss
...
def openmoe_mlp_run():
    # ================= OpenMoeMLP =================
    model = SSSparseMLP(SparseMLP(num_experts=config.num_experts,
    ...
    if args.time:
        ...
        print("OpenMoE,mlp,Samoyeds,%d,%d,%d,%d,%d,%d,%s,%s" %
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/OpenMoE_Samoyeds_MLP_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
...
def openmoe_layer_run():
    ...
    model = sparsemoeblock_to_ss(model).half().cuda()
    model.eval()
    ...
    if args.time:
        ...
        print("OpenMoE,layer,Samoyeds,%d,%d,%d,%d,%d,%d,%s,%s" %
              (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size, args.experts, (end - start) * 1000, config._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            schedule=torch.profiler.schedule(wait=1, warmup=10, active=10, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/OpenMoE_Samoyeds_layer_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
                    0] + '_' + str(args.batch_size)),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
