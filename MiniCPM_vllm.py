...
from MiniCPM.modeling_MiniCPM_vllm import MiniCPMModel, MiniCPMDecoderLayer, MiniCPMMoE
...
def MiniCPM_mlp_run():
    dense_model = MiniCPMMoE(expert_num, 2, k, m, torch.half, 1).half().cuda()
    dense_model.eval()
    ...
    if args.time:
        ...
        print("MiniCPM,mlp,vLLM,%d,%d,%d,%d,%d,%d,%s,%s" %
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/MiniCPM_vLLM_MLP_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
...
def MiniCPM_decoder_layer_run():
    ss_model = MiniCPMDecoderLayer(configuration, 0).half().cuda()
    ss_model.eval()
    ...
    if args.time:
        ...
        print("MiniCPM,layer,vLLM,%d,%d,%d,%d,%d,%d,%s,%s" %
              (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size, args.experts, (end - start) * 1000, configuration._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            schedule=torch.profiler.schedule(wait=1, warmup=10, active=10, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/MiniCPM_vLLM_Layer_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
                    0] + '_' + str(args.batch_size)),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
...
def MiniCPM_model_run():
    ...
    if args.time:
        ...
        print("MiniCPM,model,vLLM,%d,%d,%d,%d,%d,%d,%s,%s" %
              (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size, args.experts, (end - start) * 1000, configuration._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/MiniCPM_vLLM_model_' + torch.cuda.get_device_name().split(' ')[1].split('-')[0]),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
        )
