...
from vllm.model_executor.models.mixtral import MixtralMoE, MixtralDecoderLayer
...
def qwen2_moe_mlp_run():
    ...
    ss_model = Qwen2MoeSparseMoeBlock(configuration).cuda()
    ss_model.eval()
    ...
    print("qwen2_moe,mlp,vLLM,%d,%d,%d,%d,%d,%d,%s,%s" %
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/qwen2_moe_vLLM_MLP_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
...
def qwen2_moe_decoder_layer_run():
    torch.set_default_dtype(torch.half)
    ss_model = Qwen2MoeDecoderLayer(configuration, 0).cuda()
    ss_model.eval()
    ...
    print("qwen2_moe,layer,vLLM,%d,%d,%d,%d,%d,%d,%s,%s" %
          (ITER, args.batch_size, args.seq_len, args.hidden_size, args.intermediate_size, args.experts, (end - start) * 1000, configuration._attn_implementation))
    ...
    if args.profile:
        prof = torch.profiler.profile(
            ...
            schedule=torch.profiler.schedule(wait=1, warmup=10, active=10, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(
                './outputs/profiler/qwen2_moe_vLLM_Layer_' + torch.cuda.get_device_name().split(' ')[1].split('-')[
                    0] + '_' + str(args.batch_size)),
            record_shapes=True,
            profile_memory=True,
            with_stack=True
