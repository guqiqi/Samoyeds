...
class SSMiniCPMMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.w1 = SSFusedSiluTransLinear(nn.Linear(self.hidden_size, self.intermediate_size, bias=False))
        self.w3 = SSTransLinear(nn.Linear(self.hidden_size, self.intermediate_size, bias=False))
        self.w2 = SPDenseWeightedLinear(nn.Linear(self.intermediate_size, self.hidden_size, bias=False))
...
class SSMiniCPMMoE(nn.Module):
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList(
            [SSMiniCPMMLP(config) for i in range(self.num_experts)]
        )
...
class SSMiniCPMDecoderLayer(nn.Module):
    def __init__(self, config: MiniCPMConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = MINICPM_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = SSMiniCPMMoE(config)
...
class SSMiniCPMModel(MiniCPMPreTrainedModel):
    """
    ...
    def __init__(self, config: MiniCPMConfig):
        ...
        self.layers = nn.ModuleList(
            [SSMiniCPMDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
