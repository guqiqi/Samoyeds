...
class OpenMoeAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, use_flash: bool = False):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        if HAS_FLASH_ATTN and self.use_flash:
...
class OpenMoeDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, moe: bool, use_flash: bool=False):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.moe = moe
        self.self_attn = OpenMoeAttention(config=config, use_flash=use_flash)  
